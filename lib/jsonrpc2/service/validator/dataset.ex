defmodule JSONRPC2.Service.Validator.Dataset do
  @type dataset :: t() | map()
  ...
  defp error_messages([{key, key_errors} | errors], messages) do
    key_messages = Enum.map(key_errors, fn({msg, opts}) -> dgettext(msg, opts) end)
    error_messages(errors, [{key, key_messages} | messages])
  end

  defp dgettext(msg, opts) do
    Gettext.dgettext(JSONRPC2.Service.Gettext, "errors", msg, opts)
  end
end
